...
/ (корень проекта)
  ...
  |-- pages/                    # Nuxt-страницы приложения (маршруты)
        ...
        |-- apply-teacher.vue # Страница заявки на роль преподавателя
        |-- instructors/          # Страницы преподавателей
            |-- [id].vue          # Профиль преподавателя
        |-- index.vue             # Главная страница
        |-- login.vue             # Страница входа
      |-- register.vue          # Страница регистрации
      |-- app/
          |-- index.vue         # Объединенный дашборд (роль-адаптивный: студенты/преподаватели)
          |-- settings.vue      # Настройки профиля (роль-адаптивные: +bio для преподавателей)
          |-- my-courses.vue    # Мои курсы (роль-адаптивные: студенты/преподаватели)
          |-- my-instructors.vue # Список преподавателей
          |-- wishlist.vue      # Список желаемого пользователя
          |-- courses/
              |-- index.vue     # Список всех курсов
              |-- [id].vue      # Страница курса (детали)
              |-- new.vue       # Создание нового курса (только преподаватели)
              |-- [courseId]/
                  |-- edit.vue  # Редактирование курса (только преподаватели)
                  |-- modules/
  ...
  |-- stores/
      |-- wishlist.ts            # Pinia store для wishlist, используется в wishlist.vue

## Роль-адаптивные страницы

Структура страниц организована с учетом ролей пользователей:

### Основные принципы:
1. **Единая точка входа**: `/app/` для всех пользователей
2. **Роль-адаптивность**: Один компонент показывает разный интерфейс в зависимости от роли
3. **Логичная группировка**: Все связанное с приложением в `/app/`

### Ключевые страницы:

| Маршрут | Описание | Роль-адаптивность |
|---------|----------|-------------------|
| `/app/index.vue` | Главная страница приложения | ✅ Студенты: статистика + курсы<br>✅ Преподаватели: аналитика + курсы |
| `/app/settings.vue` | Настройки профиля | ✅ Все: основные поля<br>✅ Преподаватели: +поле "О себе" |
| `/app/my-courses.vue` | Мои курсы | ✅ Студенты: курсы с прогрессом<br>✅ Преподаватели: ведомые курсы |
| `/app/my-instructors.vue` | Список преподавателей | ❌ Единый для всех |
| `/app/wishlist.vue` | Список желаемого | ❌ Только для студентов |
| `/app/courses/new.vue` | Создание курса | ❌ Только для преподавателей |
| `/app/courses/[courseId]/edit.vue` | Редактирование курса | ❌ Только для преподавателей |
...
| **pages/app/index.vue** | Роль-адаптивная главная страница | Nuxt routing, основная навигация |
| **pages/app/settings.vue** | Настройки профиля (роль-адаптивные) | Основные поля + bio для преподавателей |
| **pages/app/my-courses.vue** | Мои курсы (роль-адаптивные) | Курсы студентов/преподавателей |
...
| **stores/wishlist.ts** | Pinia store для wishlist | Используется в wishlist.vue |
